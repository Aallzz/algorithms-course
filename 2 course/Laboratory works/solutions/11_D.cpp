...
long long dfs(int v, long long flow, long long allow) {
    if (!flow) return 0ll;
    if (v == t) return flow;
    for (; ptr[v] < (int)g[v].size(); ++ptr[v]) {
        ...
        long long pushed = dfs(cur.to, min(flow, cur.cap - cur.flow), allow);
        if (pushed) {
            cur.flow += pushed;
 ...
 long long solve() {
    long long flow {};
    for (long long allow = 1ll << 31ll; ; allow >>= 1ll) {
        while (bfs(allow)) {
            fill_n(ptr, n, 0);
            while (long long pushed = dfs(s, inf, allow)) flow += pushed;
        }
        if (!allow) break;
    }
    return flow;
